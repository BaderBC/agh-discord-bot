 ...
 */
import type { GeneratedIds } from '../config/ids.js';
import type { Kierunek } from '../config/kierunki.js';
...
export function buildRegistryFromData(ids: GeneratedIds | null, courses: Kierunek[]): Registry {
  if (!ids) {
  ...
  const kierunki = buildKierunki(ids, courses);
...
function buildKierunki(ids: GeneratedIds, courses: Kierunek[]): KierunekOption[] {
  const emojiBySlug = new Map(courses.map((k) => [k.slug, k.emoji]));
